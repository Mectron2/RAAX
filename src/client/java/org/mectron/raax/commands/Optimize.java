...
public class Optimize extends Base {
    ...
    public void run(String[] args) {
        if (args.length < 2) {
            ...
            MinecraftClient.getInstance().player.sendMessage(Text.of("[RAAX] Please provide a ore block to optimize for as argument. Currently: diamond, redstone ore or stone"), false);
            return;
        }
        ...
        switch (newblock) {
            case "diamond":
                ...
                MinecraftClient.getInstance().player.sendMessage(Text.of("[RAAX] Set optimization to diamond"), false);
                break;

            case "redstone":
                Config.checkblocks = new Block[]{Blocks.REDSTONE_ORE};
                MinecraftClient.getInstance().player.sendMessage(Text.of("[RAAX] Set optimization to redstone"), false);
                break;

            case "stone":
                Config.checkblocks = new Block[]{Blocks.STONE};
                MinecraftClient.getInstance().player.sendMessage(Text.of("[RAAX] Set optimization to stone"), false);
                break;
            ...
            default:
                ...
                MinecraftClient.getInstance().player.sendMessage(Text.of("[RAAX] optimization reset to all ores"), false);
        }
